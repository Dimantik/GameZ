package dnt.dimantik.md.gamez.game.logic.game.settings;
...
import dnt.dimantik.md.gamez.game.logic.interfaces.MapInterface;
import dnt.dimantik.md.gamez.game.logic.interfaces.PlayerInterface;
import dnt.dimantik.md.gamez.game.logic.interfaces.ResourceInterface;

import static dnt.dimantik.md.gamez.game.logic.game.settings.AssertResourceName.*;

public class GameCreator {

    public static void createNewGame(GameInterface gameInterface, String playerName){
        BDHelper.deleteGame();
    ...
    private static void addNewMap(GameInterface gameInterface){
        ...
        Location location1 = Location.createNewLocation(1, "Город Z",
                "Раньше это был огромный мегаполис, теперь же это город мертвых", LOCATION_CITY_Z);

        Place place11 = Place.createNewPlace(location1.getNewIdFromNewPlace(), "Продуктовый магазин", 1, 2, "Тут должно быть много продуктов", PLACE_MARKET, resourceTypesFromRoom);
        location1.addPlace(place11, true);
        Place place12 = Place.createNewPlace(location1.getNewIdFromNewPlace(), "Заправка", 2, 2, "Может посчастливиться найти машину на ходу", PLACE_GAS_STATION, resourceTypesFromOutside);
        location1.addPlace(place12, true);
        Place place13 = Place.createNewPlace(location1.getNewIdFromNewPlace(), "Больница", 3, 3, "Возможно тут еще остались антибиотики", PLACE_HOSPITAL, resourceTypesFromAll);
        location1.addPlace(place13, true);
        Place place14 = Place.createNewPlace(location1.getNewIdFromNewPlace(), "Полицейский участок", 2, 6, "Вполне возможно что что-то из оружия еще осталось", PLACE_POLICE_STATION, resourceTypesFromAll);
        location1.addPlace(place14, true);
        Place place15 = Place.createNewPlace(location1.getNewIdFromNewPlace(), "Военая база", 2, 10, "Без сомнений тут можно найти немало полезного", PLACE_MILITARY_STATION, resourceTypesFromAll);
        location1.addPlace(place15, true);
        Place place16 = Place.createNewPlace(location1.getNewIdFromNewPlace(), "Гипермаркет", 1, 4, "Тут должно быть много провизии", PLACE_S_E_CENTER, resourceTypesFromRoom);
        location1.addPlace(place16, true);



        Location location2 = Location.createNewLocation(2, "Заповедник",
                "Это место представляет из себя огромный заповедник, раньше тут водилось множество видов животных, сейчас же никто не знает что там.", LOCATION_RESERVE);

        Place place21 = Place.createNewPlace(location2.getNewIdFromNewPlace(), "Домик лесника", 0, 1, "Может быть владелец дома еще жив?", PLACE_FORESTER_HOUSE, resourceTypesFromRoom);
        location2.addPlace(place21, true);
        Place place22 = Place.createNewPlace(location2.getNewIdFromNewPlace(), "Вальеры с животными", 0, 1, "Кто знает что тут может быть", PLACE_FOREST, resourceTypesFromOutside);
        location2.addPlace(place22, true);
...
//        game.getPlaceLevelOneList().add(place5);
        ...
        Way way = new Way(11, 12, 70);
        wayList.add(way);
        way = new Way(11, 13, 30);
        wayList.add(way);
        way = new Way(11, 14, 60);
        wayList.add(way);
        way = new Way(11, 15, 90);
        wayList.add(way);
        way = new Way(11, 16, 80);
        wayList.add(way);
        way = new Way(12, 13, 120);
        wayList.add(way);
        way = new Way(12, 14, 60);
        wayList.add(way);
        way = new Way(12, 15, 240);
        wayList.add(way);
        way = new Way(12, 16, 90);
        wayList.add(way);
        way = new Way(13, 14, 70);
        wayList.add(way);
        way = new Way(13, 15, 50);
        wayList.add(way);
        way = new Way(13, 16, 30);
        wayList.add(way);
        way = new Way(14, 15, 100);
        wayList.add(way);
        way = new Way(14, 16, 20);
        wayList.add(way);
        way = new Way(15, 16, 70);
        wayList.add(way);
        way = new Way(21, 22, 30);
        wayList.add(way);
        way = new Way(1, 2, 400);
        wayList.add(way);
        ...
        locationMap.put(location2.getId(), location2);
        ...
        placeMap.put(place21.getId(), place21);
        placeMap.put(place22.getId(), place22);
    ...
    private static void addNewPlayer(GameInterface gameInterface, String name){
        ...
        Transport transport = new Transport("Военный джип", 0.25, 20, 1000, 5, TRANSPORT_1_IMG);
    ...
    private static void addNewResourceList(GameInterface gameInterface){
        ...
        Transport transport1 = new Transport("Военный джип", 0.05, 20, 1000, 5, TRANSPORT_1_IMG);
        transport1.setBag(bagTransport1, true);
        resourceMapToPuToPlace.put(transport1.getId(), transport1);

        Transport transport2 = new Transport("Легковой автомобиль", 0.1, 10, 1000, 3, TRANSPORT_1_IMG);
        transport2.setBag(bagTransport2, true);
        resourceMapToPuToPlace.put(transport2.getId(), transport2);

        Transport transport4 = new Transport("Мопед", 0.3, 1, 1000, 1, TRANSPORT_1_IMG);
        ...
        Transport transportTest = new Transport("Военный джип", 0.05, 20, 1000, 5, TRANSPORT_1_IMG);
    ...
    public static void continueGame(GameInterface gameInterface){
    ...
    private static void addCurrentMap(GameInterface gameInterface){
        ...
        for (Location location : locationMap.values()){
            Log.i("NEW", location.getName() + " PLACE SIZE - " + location.getPlaceIdList().size());
