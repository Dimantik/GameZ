package dnt.dimantik.md.gamez.game.logic.game.settings;

/**
 * Created by dimantik on 13.03.2018.
 */

public class AssertResourceName {

    public static final String PLAYER_IMG = "player_img.";

    public static final String SKULL = "tools/skull.png";

    public static final String GAME_OVER = "tools/game_over.jpg";

    public static final String BAG_1_IMG = "capacity/bag_1_img.png";
    public static final String BAG_2_IMG = "capacity/bag_2_img.png";
    public static final String BAG_3_IMG = "capacity/bag_3_img.png";
    public static final String BAG_TRANSPORT_IMG = "bag_transport_img.";

    public static final String BODY_CLOTHES_1_IMG = "clothes/body_clothes/body_clothes_1_img.png";
    public static final String BODY_CLOTHES_2_IMG = "clothes/body_clothes/body_clothes_2_img.png";
    public static final String BODY_CLOTHES_3_IMG = "clothes/body_clothes/body_clothes_3_img.png";

    public static final String HEAD_CLOTHES_1_IMG = "clothes/head_clothes/head_clothes_1_img.png";
    public static final String HEAD_CLOTHES_2_IMG = "clothes/head_clothes/head_clothes_2_img.png";
    public static final String HEAD_CLOTHES_3_IMG = "clothes/head_clothes/head_clothes_3_img.png";

    public static final String LEGS_CLOTHES_1_IMG = "clothes/legs_clothes/legs_clothes_1_img.png";
    public static final String LEGS_CLOTHES_2_IMG = "clothes/legs_clothes/legs_clothes_2_img.png";
    public static final String LEGS_CLOTHES_3_IMG = "clothes/legs_clothes/legs_clothes_3_img.png";

    public static final String FEET_CLOTHES_1_IMG = "clothes/feet_clothes/feet_clothes_1_img.png";
    public static final String FEET_CLOTHES_2_IMG = "clothes/feet_clothes/feet_clothes_2_img.png";
    public static final String FEET_CLOTHES_3_IMG = "clothes/feet_clothes/feet_clothes_3_img.png";

    public static final String DRUG_1_IMG = "medicines/drug_1_img.png";
    public static final String DRUG_2_IMG = "medicines/drug_2_img.png";
    public static final String DRUG_3_IMG = "medicines/drug_3_img.png";

    public static final String FOOD_1_IMG = "food/food_1_img.png";
    public static final String FOOD_2_IMG = "food/food_2_img.png";
    public static final String FOOD_3_IMG = "food/food_3_img.png";

    public static final String LIQUID_1_IMG = "food/liquid_1_img.png";
    public static final String LIQUID_2_IMG = "food/liquid_2_img.png";
    public static final String LIQUID_3_IMG = "food/liquid_3_img.png";

    public static final String FIRE_ARMS_BOW_1_IMG = "ranged_weapons/ranged_weapon_bow_1_img.png";
    public static final String FIRE_ARMS_BOW_2_IMG = "ranged_weapons/ranged_weapon_bow_2_img.png";
    public static final String FIRE_ARMS_PISTOL_1_IMG = "ranged_weapons/ranged_weapon_pistol_1_img.png";
    public static final String FIRE_ARMS_PISTOL_2_IMG = "ranged_weapons/ranged_weapon_pistol_2_img.png";
    public static final String FIRE_ARMS_PISTOL_3_IMG = "ranged_weapons/ranged_weapon_pistol_3_img.png";
    public static final String FIRE_ARMS_PISTOL_4_IMG = "ranged_weapons/ranged_weapon_pistol_4_img.png";
    public static final String FIRE_ARMS_SHOT_GUN_1_IMG = "ranged_weapons/ranged_weapon_shot_gun_1_img.png";
    public static final String FIRE_ARMS_SHOT_GUN_2_IMG = "ranged_weapons/ranged_weapon_shot_gun_2_img.png";
    public static final String FIRE_ARMS_SUBMACHINE_GUN_1_IMG = "ranged_weapons/ranged_weapon_submachine_gun_1_img.png";
    public static final String FIRE_ARMS_SUBMACHINE_GUN_2_IMG = "ranged_weapons/ranged_weapon_submachine_gun_2_img.png";
    public static final String FIRE_ARMS_SUBMACHINE_GUN_3_IMG = "ranged_weapons/ranged_weapon_submachine_gun_3_img.png";
    public static final String FIRE_ARMS_SUBMACHINE_GUN_4_IMG = "ranged_weapons/ranged_weapon_submachine_gun_4_img.png";
    public static final String FIRE_ARMS_MACHINE_1_IMG = "ranged_weapons/ranged_weapon_machine_1_img.png";
    public static final String FIRE_ARMS_MACHINE_2_IMG = "ranged_weapons/ranged_weapon_machine_2_img.png";
    public static final String FIRE_ARMS_MACHINE_GUN_1_IMG = "ranged_weapons/ranged_weapon_machine_gun_1_img.png";
    public static final String FIRE_ARMS_MACHINE_GUN_2_IMG = "ranged_weapons/ranged_weapon_machine_gun_2_img.png";
    public static final String FIRE_ARMS_SNIPER_RIFLE_1_IMG = "ranged_weapons/ranged_weapon_sniper_rifle_1_img.png";
    public static final String FIRE_ARMS_SNIPER_RIFLE_2_IMG = "ranged_weapons/ranged_weapon_sniper_rifle_2_img.png";
    public static final String FIRE_ARMS_SNIPER_RIFLE_3_IMG = "ranged_weapons/ranged_weapon_sniper_rifle_3_img.png";

    public static final String CARTRIDGES_PISTOL_IMG = "ranged_weapons/cartridges_pistol_img.png";
    public static final String CARTRIDGES_SHOT_GUN_IMG = "ranged_weapons/cartridges_shot_gun_img.png";
    public static final String CARTRIDGES_SUBMACHINE_GUN_IMG = "ranged_weapons/cartridges_submachine_gun_img.png";
    public static final String CARTRIDGES_MACHINE_IMG = "ranged_weapons/cartridges_machine_img.png";
    public static final String CARTRIDGES_MACHINE_GUN_IMG = "ranged_weapons/cartridges_machine_gun_img.png";
    public static final String CARTRIDGES_SNIPER_RIFLE_IMG = "ranged_weapons/cartridges_sniper_rifle_img.png";

    public static final String STEEL_ARMS_1_IMG = "melee_weapons/steel_arms_1_img.png";
    public static final String STEEL_ARMS_2_IMG = "melee_weapons/steel_arms_2_img.png";
    public static final String STEEL_ARMS_3_IMG = "melee_weapons/steel_arms_3_img.png";

    public static final String TRANSPORT_1_IMG = "transport/transport_1_img.png";
    public static final String TRANSPORT_2_IMG = "transport_2_img.png";
    public static final String TRANSPORT_3_IMG = "transport_3_img.png";


    public static final String LOCATION_CITY_Z = "place/city_z.jpg";
    public static final String LOCATION_RESERVE = "place/reserve.jpg";


    public static final String PLACE_GAS_STATION = "place/gas_station.jpg";
    public static final String PLACE_HOSPITAL = "place/hospital.jpg";
    public static final String PLACE_MARKET = "place/market.jpg";
    public static final String PLACE_MILITARY_STATION = "place/military_station.jpg";
    public static final String PLACE_POLICE_STATION = "place/police_station.jpg";
    public static final String PLACE_S_E_CENTER = "place/s_e_center.jpg";
    public static final String PLACE_FORESTER_HOUSE = "place/forester_house.jpg";
    public static final String PLACE_FOREST = "place/forest.jpg";

    public static final String PROTECTION = "skills/protection.png";
    public static final String POWER = "skills/power.png";
    public static final String MURDER_SKILL = "skills/murder_skill.png";

    public static final String MAIN_HERO = "characters/main_hero.png";
}
